...
class BluetoothClient(toga.App):
    ...
    def _create_left_panel(self) -> None:
        ...
        self.connect_button = toga.Button(
            "蓝牙连接",
            style=Pack(padding=5),
            on_press=self.connect_bluetooth
        )
        ...
        self.disconnect_button = toga.Button(
            "蓝牙断开",
            style=Pack(padding=5),
            on_press=self.disconnect_bluetooth
        )
    ...
    def _create_control_panels(self) -> None:
        ...
        self.light_row.add(  # type: ignore
            ...
            *self._create_control_buttons([1, 2, 3, 4, 5, 6], 
                ["开", "关", "调亮", "调暗", "呼吸", "流水"])
        )
        ...
        self.fan_row.add(  # type: ignore
            ...
            *self._create_control_buttons([7, 8, 9, 10], 
                ["开", "关", "加速", "减速"])
        )
        ...
        self.heater_row.add(  # type: ignore
            ...
            *self._create_control_buttons([15, 16, 17, 18], 
                ["开", "关", "升温", "降温"])
        )
    ...
    def _create_control_buttons(
        ...
        return [
            toga.Button(
                label,
                style=Pack(padding=5),
                on_press=self.create_control_handler(cmd)
            )
    ...
    def _create_status_table(self) -> None:
        ...
        self.device_status.add(  # type: ignore
            toga.Label("设备状态", style=Pack(padding=(7, 5))),
            self.status_table
        )
        ...
        async def start_scan() -> None:
            ...
            try:
                ...
                matched = [
                    ...
                    for d in devices
                    if d.name and (self.target_name in d.name or self.is_esp_device(d.address))
                ]
                ...
                await self.main_window.dialog(toga.ErrorDialog("错误", f"扫描失败: {e}"))
    ...
    def connect_bluetooth(self, widget: toga.Widget) -> None:
        ...
        async def start_connect() -> None:
            if not self.device_table or not self.device_table.selection:
                await self.main_window.dialog(toga.ErrorDialog("错误", "请先选择要连接的设备"))
                return
            ...
            try:
                ...
                await self.main_window.dialog(toga.ErrorDialog("错误", f"连接异常: {str(e)}"))
        ...
        async def start_disconnect() -> None:
            if self.client and self.client.is_connected:
                try:
                    ...
                    await self.main_window.dialog(toga.ErrorDialog("错误", f"断开失败: {e}"))
            else:
                await self.main_window.dialog(toga.InfoDialog("提示", "当前没有已连接的设备"))
